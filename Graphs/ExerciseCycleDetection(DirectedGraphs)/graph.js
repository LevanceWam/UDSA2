...
const topological = Symbol();
const hasCycleAlgorithm = Symbol();
...
class Graph{
    ...
    depthFirstRecursive(root){
        ...
        return this[depthRecursive](node, new Set());
    }
    ...
    breadthFirstIteration(root){
        ...
        while(!queue.isEmpty()){
            ...
            for(let neighbors of this.adjencyList.get(current)){
                if(!visited.has(neighbors)) queue.enqueue(neighbors);
            }
        }
    }
    ...
    topologicalSort(){
        ...
        for (let node of this.map.values()){
            this[topological](node, visited, stack);
        }
    ...
    hasCycle(){
        const all = new Set();
        all.add(this.map.values().next().value);
        ...
        while(!all.size == 0){
            ...
            if (this[hasCycleAlgorithm](current, all, visiting, visited))
                return true;
        }
    ...
    [topological](node, visited, stack){
        ...
        for (let neighbor of this.adjencyList.get(node)){
            this[topological](neighbor,visited,stack);
        }

        stack.push(node);
    }

    [hasCycleAlgorithm](node, all, visiting, visited){
        all.delete(node);
        visiting.add(node);
        ...
        for (let neighbor of this.adjencyList.get(node)){
            ...
            if (this[hasCycleAlgorithm](neighbor, all, visiting, visited)) return true;
        }
